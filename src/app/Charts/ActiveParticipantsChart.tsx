...
const monthlyData = [9836, 870047, 1571043, 2067822, 2693455, 3140922, 3376680, 3687612, 4076770, 4561988, 5398672, 6281094, 7034910, 7743910, 8469040, 9581094, 10584901, 11485109, 12567903]
...
function generateDailyData(monthlyData: number[]) {
    ...
    for (let j = 0; j < lastMonthDays; j++) {
        previousValue += dailyIncrement - 30000;
...
function generateWeeklyData(monthlyData: number[]) {
    ...
    for (let j = 0; j < weeksInLastMonth - 1; j++) {
        previousValue += weeklyIncrement - 220000;
        increments.push(Math.round(previousValue));
    }
...
export default function ActiveParticipantsChart({ timeframe }: { timeframe: "Day" | "Week" | "Month" }) {

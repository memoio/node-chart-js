...
const monthlyData = [1089, 2829, 4530, 6289, 7906, 9673, 11324, 13098, 14703, 16451, 18100, 18841, 20509, 23570, 26790, 29598, 34078, 41008, 47975
]
...
function generateDailyData(monthlyData: number[]) {
    ...
    for (let j = 0; j < lastMonthDays; j++) {
        previousValue += dailyIncrement - 200;
...
function generateWeeklyData(monthlyData: number[]) {
    ...
    for (let j = 0; j < weeksInLastMonth - 1; j++) {
        previousValue += weeklyIncrement - 1000;
        increments.push(Math.round(previousValue));
    }
...
export default function ProviderNodesChart({ timeframe }: { timeframe: "Day" | "Week" | "Month" }) {
    const dataLength = ProviderNodeData[timeframe].length;
    ...
    useEffect(() => {
        setSeriesData([
            {
                name: 'Provider Nodes',
                data: ProviderNodeData[timeframe], // 动态选择数据
            }
        ]);
        setCategories(generateDates(timeframe, ProviderNodeData[timeframe].length));
    }, [timeframe]);
